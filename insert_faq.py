...
conn = mysql.connector.connect(host='localhost', user='root', password='root', database='linmeng')
cursor = conn.cursor()
...
cursor.execute("INSERT INTO sys_configs (config_key, config_value, config_type, description) VALUES ('service_faqs', %s, 'service', '智能客服FAQ') ON DUPLICATE KEY UPDATE config_value = VALUES(config_value)", (faqs_json,))
conn.commit()
print('FAQ插入成功')

# 验证
cursor.execute("SELECT config_key, LEFT(config_value, 50) FROM sys_configs WHERE config_key = 'service_faqs'")
result = cursor.fetchone()
if result:
    print(f"已存在: {result[0]} - {result[1]}")
