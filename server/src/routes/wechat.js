...
/**
 * 微信账号绑定手机号（扫码登录后未绑定时使用）
 * POST /api/wechat/bind
 * Body: { openid, unionid, user_type, phone, userId }
 */
router.post('/bind', async (req, res) => {
  try {
    const { openid, unionid, user_type, phone, userId } = req.body
    if (!openid || !user_type || !userId) {
      return res.json({ code: 400, msg: '缺少必要参数' })
    }
    await db.query(
      'INSERT IGNORE INTO wechat_user_bind (unionid, openid, user_type, user_id, phone) VALUES (?, ?, ?, ?, ?)',
      [unionid || '', openid, user_type, userId, phone || '']
    )
